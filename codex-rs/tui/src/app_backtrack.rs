...
//! to edit an earlier prompt. Confirming a transcript selection opens a second selection for
//! rolling the current thread back or forking before the selected turn, then restores its prompt
//! in the composer.
//!
...
impl App {
    ...
    pub(crate) fn apply_backtrack_selection(&mut self, selection: BacktrackSelection) {
        ...
        let rollback_nth_user_message = selection.nth_user_message;
                ...
                SelectionItem {
                    ...
                    actions: vec![Box::new(move |tx| {
                        tx.send(AppEvent::EditEarlierPrompt {
                            ...
                            nth_user_message: rollback_nth_user_message,
...
/// reopened independently because app-server cannot cut history in the middle of a turn.
pub(crate) fn backtrack_turn_target(
    ...
    for (turn_index, turn) in turns.iter().enumerate() {
        ...
        for item in &turn.items {
            ...
            return Ok(BacktrackTurnTarget {
                before_turn_id: turn.id.clone(),
                has_loaded_turn_before: turn_index > 0,
                turns_to_remove: u32::try_from(turns.len().saturating_sub(turn_index))
                    .unwrap_or(u32::MAX),
            });
        }
    }
...
mod tests {
    ...
    #[test]
    fn backtrack_turn_target_resolves_first_and_later_prompts() {
        let turns = vec![
    ...
    #[test]
    fn backtrack_turn_target_rejects_mid_turn_steers() {
        let turns = vec![turn(
            "turn-1",
            TurnStatus::Completed,
            /*user_messages*/ 2,
        ...
        let error = backtrack_turn_target(
            &turns,
            /*nth_user_message*/ 1,
    ...
    #[test]
    fn backtrack_turn_target_rejects_in_progress_and_missing_prompts() {
        let turns = vec![turn(
            "turn-1",
    ...
    #[test]
    fn backtrack_turn_target_skips_hidden_review_prompts() {
    ...
    #[test]
    fn backtrack_turn_target_skips_hidden_nested_review_prompts() {
    ...
    #[test]
    fn backtrack_turn_target_restores_canonical_mention_bindings() {
        ...
        assert_eq!(
            backtrack_turn_target(&turns, /*nth_user_message*/ 1, &mut selected_prompt,)
