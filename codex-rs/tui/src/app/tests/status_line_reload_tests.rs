...
async fn status_line_reloads_follow_accepted_reads_and_survive_thread_switches() -> Result<()> {
    ...
    let mut count_only = response.clone();
    count_only
        .rate_limit_reset_credits
        .as_mut()
        .expect("test response should include reset credits")
        .credits = None;
    app.handle_event(
        &mut tui,
        &mut session,
    ...
    for (request_id, hard_stop_generation, result) in [
        ...
        (4, generation, Err("account read failed".to_string())),
        (5, generation.wrapping_add(1), Ok(without_credits.clone())),
    ] {
        app.handle_event(
            &mut tui,
            &mut session,
